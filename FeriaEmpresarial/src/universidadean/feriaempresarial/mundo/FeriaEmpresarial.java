...
public class FeriaEmpresarial
{
    ...
    /* cantidad de expositores en un puesto
    ...
    {
        //Primero encontar dónde está el puesto
        Puesto p = null;
        for (Puesto puesto : puestos){
    ...
    public String metodo1()
    {
        ...
        {
            if (empr.darTipo().equals(Empresa.EXPOSITORA))
            {
                for (Puesto p:puestos)
                {
                    if (p.estaOcupado())
                    {
                        if (p.darNombreEmpresa().equals(empr.darNombre()))
                        {
                            if (zc >= zn && zc >=zs && zc >=zoc && zc >= zor)
                            {
                                return "La Zona del Centro tiene mas ocupación, revisa su aforo";
                            }
                            else if (zn >= zc && zn >=zs && zn >=zoc && zn >= zor)
                            {
                                return "La Zona del Norte tiene mas ocupación, revisa su aforo";
                            }
                            else if (zs >= zc && zs >=zn && zs >=zoc && zs >= zor)
                            {
                                return "La Zona del Sur tiene mas ocupación, revisa su aforo";
                            }
                            else if (zoc >= zc && zoc >=zn && zoc >=zs && zoc >= zor)
                            {
                                return "La Zona Occidental tiene mas ocupación, revisa su oforo";
                            }
                            else if (zor >= zc && zor >=zn && zor >=zs && zor >= zoc)
                            {
                                return "La Zona Oriental tiene mas ocupación, revisa su aforo";
                            }
                        }
                    }
                }
        ...
        return "no hay empresas expositoras registradas";
    }
    ...
    public String metodo2()
    {
        ...
        for (Puesto p : puestos)
        {
            t_exp = t_exp + p.darNumMaximoPersonas();
        }
        for (Empresa empr : empresas)
        {
            if (empr.darTipo().equals(Empresa.EXPOSITORA))
            {
                for (Puesto p : puestos)
                {
                    if (p.estaOcupado())
                    {
                        if (p.darNombreEmpresa().equals(empr.darNombre()))
                        {
                            n_exp = n_exp + empr.darNumeroPersonasAsistentes();
                        }
                    }
                }
            }
        }
        porcentaje = (float) n_exp / t_exp;
        if(porcentaje * 100 == 60)
        {
