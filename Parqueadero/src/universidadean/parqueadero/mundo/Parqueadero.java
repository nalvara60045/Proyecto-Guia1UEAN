...
public class Parqueadero {
    ...
    public void avanzarHora()
    {
        ...
        if (horaActual == 21)
        {
            //Informa que ya se cerro el parqueadero
            abierto = false;
        }
    }
    ...
    {
        String placa_carro = null;
        int Maximo_tiempo = 0;
        for (Puesto p : puestos)
        {
            if (p.estaOcupado())
            {
                Carro localizar_carro = puestos[p.darNumeroPuesto()].darCarro();
                if (localizar_carro.darTiempoEnParqueadero(horaActual) > Maximo_tiempo)
                {
                    Maximo_tiempo= localizar_carro.darTiempoEnParqueadero(horaActual);
                    placa_carro = localizar_carro.darPlaca();
                }
            }
        }
        return "El carro con placa " + placa_carro + " es el que lleva más tiempo en nuestro parqueadero";
    }
    ...
    {
        int puesto_ocupado = 0;
        int tot_puesto = puestos.length;
        double porcentaje = 0;
        for (Puesto p : puestos)
        {
            if (p.estaOcupado())
        ...
        porcentaje = (double) puesto_ocupado / tot_puesto;
